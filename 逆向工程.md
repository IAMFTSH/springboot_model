```
AutoGeneratorCode主要修改以下几点（ctrl+f定位相关关键字）
```
...
注意注意无论是软删除还是自动填充，都是mybatisPlus的自动生成的方法才有效，自己的sql不会生效
    ...
    （自动填充需要写自动填充类，时间个人建议直接在数据库使用默认值）
    （如果使用数据库默认值，请删掉下面代码）
```

```java
扩展
gc.setSwagger2(true); //实体属性 Swagger2 注解 需要数据库中有注解 如果没用Swagger就改false （记得pom导包）

strategy.setEntityLombokModel(true);//是否支持Lombok（记得pom导包）
...
strategy.setVersionFieldName("version");//乐观琐配置  我们目前项目没有乐观琐，所以请删掉
```
